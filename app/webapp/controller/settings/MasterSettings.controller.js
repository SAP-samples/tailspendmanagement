sap.ui.define([
	...
	"use strict";
	return BaseController.extend("com.sap.tailspend.controller.settings.MasterSettings", {
		formatter: formatter,
		...
		onMasterPressed: function (oEvent) {
			...
			switch (sKey) {
				...
				case "fileExportSettings": {
					this.getRouter().navTo(sKey);
					break;
				}
				case "LogServiceSettings": {
					this.getRouter().navTo(sKey);
					break;
				}
				default: {
					this.getBundleText(oContext.getProperty("titleI18nKey")).then(function(sMasterElementText){
						this.getBundleText("clickHandlerMessage", [sMasterElementText]).then(function(sMessageText){
							var message = sMasterElementText + ' screen will be available soon. Please visit again later'
							MessageToast.show(message);
						//	alert("ganesh master onmaster pressed");
						});
					}.bind(this));
		...
		onGeneralButtonPress: function(oEvent){
			...
			this.getBundleText("clickHandlerMessage", [sButtonText]).then(function(sMessageText){
				MessageToast.show("Asset Record successfully saved");
				//alert("ganesh master g");
			});
